...
SVO* SVO::sample() {
	...
	root->children[5] = new SVO(2);
	root->children[5]->children[0] = new SVO(1);
	root->children[5]->children[0]->material.color = { 0, 255, 0 };

	root->children[5]->children[7] = new SVO(1);
	root->children[5]->children[7]->material.color = { 0, 0, 255 };
	root->children[5]->children[7]->material.water = 1;
	return root;
}*/
...
/*
void SVO::print_svo(SVO * n, int level) {
	//if (level > 5) return;

	for (int i = 0; i < level; i++) 
		std::cout << "    ";
	if (n == nullptr) std::cout << "[leaf]" << std::endl;
	else {
		std::cout << "[inner]" << std::endl;
		for (int i = 0; i < 8; i++)
			print_svo(n->children[i], level+1);
	}
}
*/
